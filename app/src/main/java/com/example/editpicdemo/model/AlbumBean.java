...
public class AlbumBean {
	...
	public String toString() {
		return "ImageBean{" + "folderName='" + folderName + '\'' + ", topImagePath='" + topImagePath + '\''
	...
	public static void getAllAlbumFromLocalStorage(final Context context, final AlbumListCallback completeCallback) {
		new SacMultiTask<Void, Void, ArrayList<AlbumBean>>() {
			...
			protected ArrayList<AlbumBean> doInBackground(Void... params) {
				...
				// 首先查询一共有多少张
				Cursor countCursor = mContentResolver.query(mImageUri, new String[] { "COUNT(*) " },
				...
				while (index < photoCount) {// 为了手机里有几万张照片OOM，500张为一组进行扫描相册列表
					...
					Cursor mCursor = mContentResolver.query(mImageUri, new String[] { MediaStore.Images.Media.DATA },
							MediaStore.Images.Media.MIME_TYPE + "=? or " + MediaStore.Images.Media.MIME_TYPE + "=?",
							new String[] { "image/jpeg", "image/png" }, sortOrder + limit);
					if (mCursor == null)
						return null;
	...
	public static void getAlbumPhotosFromLocalStorage(final Context context, final AlbumBean album,
		...
		new SacMultiTask<Void, Void, ArrayList<PhotoItem>>() {
			...
			protected ArrayList<PhotoItem> doInBackground(Void... params) {
				...
				if ("最近".equals(album.folderName))
					mCursor = mContentResolver.query(mImageUri, new String[] { MediaStore.Images.Media.DATA },
							MediaStore.Images.Media.MIME_TYPE + "=? or " + MediaStore.Images.Media.MIME_TYPE + "=?",
							new String[] { "image/jpeg", "image/png" }, sortOrder + " LIMIT 100");
				else
					mCursor = mContentResolver.query(mImageUri, new String[] { MediaStore.Images.Media.DATA },
							"(" + MediaStore.Images.Media.MIME_TYPE + "=? or " + MediaStore.Images.Media.MIME_TYPE
				...
				for (int i = 0; i < size; i++) {// 遍历全部图片
					...
					PhotoItem entity = new PhotoItem();
					entity.url = path;// 将图片的uri放到对象里去
					if (!"最近".equals(album.folderName) && !new File(path).getParentFile().getAbsolutePath()
							.equals(album.albumFolder.getAbsolutePath()))
						continue;// 经常会有相册相互包含的情况
			...
			@Override
			protected void onPostExecute(ArrayList<PhotoItem> photoList) {
	...
	public interface AlbumPhotosCallback {
		void onSuccess(ArrayList<PhotoItem> photos);
	}
}
