package com.example.editpicdemo.widget.pictureeditor.mosaic;
...
public class MosaicViewOld extends ViewGroup implements ScaleGestureDetector.OnScaleGestureListener{
	...
	public MosaicViewOld(Context context, AttributeSet attrs) {
		super(context, attrs);
		initImage();
		mScaleGestureDetector = new ScaleGestureDetector(context,this);
		pathMatrix = new Matrix();
		scaleFloat = new float[9];
	}
	...
	private void onPathEvent(int action, int x, int y) {
		...
		PointF pointF = scaleByPoint(x,y,mImageRect.centerX(),mImageRect.centerY(),pathScale);
		x = (int) pointF.x;
		y = (int) pointF.y;
	...
	protected PointF scaleByPoint(float targetPointX, float targetPointY, float scaleCenterX, float scaleCenterY, float scale){
		...
		matrix.getValues(scaleFloat);
		return new PointF(scaleFloat[Matrix.MTRANS_X],scaleFloat[Matrix.MTRANS_Y]);
	}
	...
	public boolean onScale(ScaleGestureDetector detector) {
		...
		pathScale = 1.0f/scaleFactor;
		/*pathMatrix.reset();
		float pathScale = 1.0f /scaleFactor;
		pathMatrix.setScale(pathScale,pathScale,mImageRect.centerX(),mImageRect.centerY());*/
		/*if (mImageRect != null){
			...
			mImageRect.left = mImageRect.left - addWidth/2;
			mImageRect.right = mImageRect.right + (addWidth - addWidth/2);
			mImageRect.top = mImageRect.top - addHeight/2;
			mImageRect.bottom = mImageRect.bottom + (addHeight - addHeight/2);
			mImageHeight = mImageRect.height();
			mImageWidth = mImageRect.width();
		}*/
