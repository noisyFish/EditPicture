package com.example.editpicdemo.widget.pictureeditor.graffiti;
...
public class DrawZoomImageView extends View {
	...
	public interface WithDrawForGraffitiListener{
		void setWithDrawState(int pathSize);
		void setToolBoardState(boolean isShow);
	}
	private WithDrawForGraffitiListener mListener;
	...
	public void setWidthDrawListener(WithDrawForGraffitiListener widthDrawListener){
		this.mListener = widthDrawListener;
	}
	...
	public void revoke(){
        ...
        if(mPathList != null && mPathList.size() > 0){
			...
			mListener.setWithDrawState(mPathList.size());
        }
	}
	/**
	 * 恢复一步
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_DOWN:
			if(event.getPointerCount() ==1){
				mListener.setToolBoardState(false);
				hasLine = true;
				float x = event.getX();
		        float y = event.getY();
		...
		case MotionEvent.ACTION_UP:
			...
			lastCenterPointX = -1;
			lastCenterPointY = -1;
			mListener.setToolBoardState(true);
			if(hasLine){
            	MyUtil.LOG_D(TAG, "保存有效线条");
            	MyUtil.LOG_V(TAG, "手指抬起了，将线条放入集合，数量"+mPathList.size());
            	mDp.ratio = totalRatio;   //保存缩放比例
	...
	private void initBitmap(Canvas canvas) {
		if (sourceBitmap != null) {
			...
			if(bitmapWidth > width && bitmapHeight > height){					//图片的宽高全部大于空间的宽高
				float ratio = Math.min(width * 1.0f / bitmapWidth, height * 1.0f / bitmapHeight);
			...
			} else if (bitmapWidth >= width || bitmapHeight >= height) {        //图片的宽高只有其中之一大于控件的宽高
