...
public class MosaicView extends ViewGroup implements ScaleGestureDetector.OnScaleGestureListener{
	...
	public interface WithDrawForMosaicListener{
		void setWithDrawState(int pathSize);
		void setToolBoardState(boolean isShow);
	}

	private WithDrawForMosaicListener mListener;
	...
	public void setWithDrawForMosaicListener(WithDrawForMosaicListener withDrawForMosaicListener){
		this.mListener = withDrawForMosaicListener;
	}
...
//		Bitmap bitmap = BitmapFactory.decodeFile(absPath);
		Bitmap bitmap = getBitmapForEditor(absPath);
		int bitmapWidth = bitmap.getWidth();
		int bitmapHeight = bitmap.getHeight();
		float scaleWidth=1;
		float scaleHeight=1;
	...
	//撤销马赛克
	public void withDrawMosaic() {
		if(mTouchPaths != null && mTouchPaths.size() > 0){
			mTouchPaths.remove(mTouchPaths.size() - 1);
			if (bmMosaicLayer != null) {
				bmMosaicLayer.recycle();
				bmMosaicLayer = null;
			}
			...
			updatePathMosaic();
			invalidate();
			mListener.setWithDrawState(mTouchPaths.size());
		}
	}
